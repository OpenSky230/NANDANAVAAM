...
async function readExperienceMeta(dir) {
  ...
  try {
    ...
    return {
      ...
      stereoPanos: Array.isArray(json.stereoPanos) ? json.stereoPanos : undefined,
      flipU: (typeof json.flipU === "boolean") ? json.flipU : undefined,
      flipX: (typeof json.flipX === "boolean") ? json.flipX : undefined,
      hotspotNavTags: (typeof json.hotspotNavTags === "boolean") ? json.hotspotNavTags : undefined,
    };
  } catch {
    return {};
  }
...
async function generateManifest() {
  ...
  for (const entry of entries) {
    ...
    // Only include directories that look like runnable experiences.
    // This avoids listing scratch folders (e.g., exports/backups) in the UI.
    try {
      await fs.access(path.join(expDir, "walkthrough.json"));
    } catch {
      continue;
    }
    ...
    experiences.push({
      ...
      order: meta.order,
      stereo,
      ...(Array.isArray(meta.stereoPanos) && meta.stereoPanos.length ? { stereoPanos: meta.stereoPanos } : {}),
      ...(typeof meta.flipU === "boolean" ? { flipU: meta.flipU } : {}),
      ...(typeof meta.flipX === "boolean" ? { flipX: meta.flipX } : {}),
      ...(typeof meta.hotspotNavTags === "boolean" ? { hotspotNavTags: meta.hotspotNavTags } : {}),
    });
  }
